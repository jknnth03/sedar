...
import { useGetAllJobbandsQuery } from "../../../features/api/masterlist/jobbandsApi";
import * as styles from "./TemplateStyles";

const BiAnnualTemplateFields = ({ isReadOnly, isCreate }) => {
  const {
    control,
    watch,
    setValue,
  ...
  const [jobBandFieldFocused, setJobBandFieldFocused] = useState(false);
  const dragItem = useRef(null);
  const dragOverItem = useRef(null);

  const jobBandId = watch("job_band_id");
  const hasJobBandId = jobBandId !== null && jobBandId !== undefined;

  React.useEffect(() => {
    setValue("type", "PERFORMANCE_EVALUATION");
    setValue("phase", "EVALUATION");
  }, [setValue]);

  const { data: jobBandsData, isLoading: isLoadingJobBands } =
    useGetAllJobbandsQuery(undefined, {
      skip: !jobBandFieldFocused && !hasJobBandId,
    });
  const jobBands = jobBandsData?.result || [];
        ...
        <Grid container spacing={1.2}>
          ...
          <Grid item xs={12} md={6}>
            <Controller
              name="job_band_id"
              control={control}
              rules={{ required: "Job Band is required" }}
              render={({ field: { onChange, value } }) => {
                const selectedJobBand =
                  jobBands.find((jb) => jb.id === value) || null;

                return (
                  <Autocomplete
                    options={jobBands}
                    value={selectedJobBand}
                    onChange={(event, newValue) => {
                      if (!isReadOnly) {
                        onChange(newValue?.id || null);
                      }
                    }}
                    onOpen={() => setJobBandFieldFocused(true)}
                    getOptionLabel={(option) => option.name || ""}
                    isOptionEqualToValue={(option, value) =>
                      option.id === value?.id
                    }
                    disabled={isReadOnly}
                    loading={isLoadingJobBands}
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        label={
                        ...
                        error={!!errors.job_band_id}
                        helperText={errors.job_band_id?.message}
                        sx={styles.textField(isReadOnly)}
                        InputProps={{
                          ...params.InputProps,
                          sx: { height: "56px" },
      ...
      <Paper sx={styles.formPaper}>
        <Box sx={styles.sectionHeaderBox}>
          <Typography variant="h6" sx={styles.sectionTitle}>
            Competency Assessment Sections
          </Typography>
          {!isReadOnly && (
            <Button
              variant="contained"
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField
                      {...field}
                      label="Section Title"
                      placeholder="e.g., Core Competency Evaluation"
                      fullWidth
                      disabled={isReadOnly}
                      sx={styles.textField(isReadOnly)}
                    />
                      ...
                      <Box
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <TextField
                              {...field}
                              placeholder="Enter competency item (e.g., ABILITY TO EFFICIENTLY ORGANIZE WORK)"
                              fullWidth
                              disabled={isReadOnly}
                              size="small"
                              sx={styles.textField(isReadOnly)}
                          ...
                          <Box
                            ...
                            <Controller
                              ...
                              render={({ field }) => (
                                <TextField
                                  {...field}
                                  placeholder="Enter behavioral indicator"
                                  fullWidth
                                  disabled={isReadOnly}
                                  size="small"
                                  sx={styles.textField(isReadOnly)}
...
export default BiAnnualTemplateFields;
