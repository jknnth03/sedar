...
} from "../../../features/api/assessment-template/biAnnualTemplateApi";
import BiAnnualTemplateFields from "./BiAnnualTemplateModalFields";
...
const formatTemplateData = (template) => {
  ...
  const ratingScale = Array.isArray(template.rating_scale)
    ? template.rating_scale.map((scale) => ({
        label: scale.label,
        value: scale.value,
  ...
  return {
    ...
    job_band_id: template.job_band?.id || template.job_band_id || null,
    rating_scale: ratingScale,
    sections: sections,
  };
};

const BiAnnualTemplateModal = ({
  open = false,
  handleClose,
  ...
  const { data: templateData, isLoading: isLoadingTemplate } =
    useGetBiAnnualTemplateByIdQuery(templateId, {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!data.job_band_id) {
        enqueueSnackbar("Job Band is required", { variant: "error" });
        return;
      }
  ...
  const getModalTitle = () => {
    const titles = {
      create: "CREATE BI-ANNUAL TEMPLATE",
      view: "VIEW BI-ANNUAL TEMPLATE",
      edit: "EDIT BI-ANNUAL TEMPLATE",
    };
    return titles[currentMode] || "BI-ANNUAL Template";
  };
  ...
  const formKey = `bi-annual-template-${
    open ? "open" : "closed"
...
export default BiAnnualTemplateModal;
