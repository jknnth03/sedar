...
import { useGetPdpTwoTaskQuery } from "../../../features/api/da-task/pdpTwoApi";
import PdpTwoModalFields from "./PdpTwoModalFields";
import PdpTwoModalPrinting from "./PdpTwoModalPrinting";
import {
  StyledDialog,
  StyledDialogTitle,
  StyledDialogContent,
...
const PdpTwoModal = ({
  ...
  const {
    ...
    error: fetchError,
    refetch,
  } = useGetPdpTwoTaskQuery(entry?.id, {
    skip: !open || !entry?.id,
    refetchOnMountOrArgChange: true,
  });

  const pdpTwoData = taskData?.result || entry;
  ...
  useEffect(() => {
    if (open && pdpTwoData) {
      if (originalMode === "view" && currentMode === "view" && !hasEditedForm) {
        ...
        loadFormData(pdpTwoData);
      }
    } else if (open && !pdpTwoData) {
      reset(getInitialFormData());
    }
  }, [open, pdpTwoData]);

  const shouldEnableEditButton = () => {
    if (!pdpTwoData) return false;
    const status = pdpTwoData.status;
    return (
      status !== "APPROVED" &&
      status !== "CANCELLED" &&
  ...
  const getModalTitle = () => {
    switch (currentMode) {
      case "view":
        return "VIEW PDP 2 SUBMISSION";
      case "edit":
        return "EDIT PDP 2 SUBMISSION";
      default:
        return "PDP 2 SUBMISSION";
    }
  };
  ...
  const showLoadingState = isProcessing && !pdpTwoData;
      ...
      <LocalizationProvider dateAdapter={AdapterDayjs}>
            ...
            <StyledDialogContent>
              {showLoadingState ? (
                ...
                <Alert severity="error" sx={{ mb: 2 }}>
                  Failed to load PDP 2 details. Please try again.
                </Alert>
              ) : pdpTwoData ? (
                <>
                  <InfoCard>
                    <Box
                      ...
                      {pdpTwoData.status && (
                        <Chip
                          label={getStatusLabel(pdpTwoData.status)}
                          sx={{
                            backgroundColor: getStatusColor(pdpTwoData.status),
                            color: "white",
                            fontWeight: 600,
                          }}
                        />
        ...
        <Dialog
          ...
          <DialogContent sx={{ p: 0, overflow: "auto" }}>
            {pdpTwoData && <PdpTwoModalPrinting data={pdpTwoData} />}
          </DialogContent>
        </Dialog>
