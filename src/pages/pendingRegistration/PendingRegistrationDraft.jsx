...
const PendingRegistrationDraft = ({
  ...
  const canApproveEmployee = useCallback((employee) => {
    ...
    const approvableStatuses = ["DRAFT", "draft"];
  ...
  const canRejectEmployee = useCallback((employee) => {
    ...
    const rejectableStatuses = ["DRAFT", "draft"];
    const currentStatus = employee?.approval_status || employee?.status;
  ...
  const handleActionConfirm = async () => {
    ...
    try {
      ...
      switch (confirmAction) {
        case "approve":
          enqueueSnackbar(
            "Approve functionality not available for draft registrations.",
            { variant: "info", autoHideDuration: 3000 },
          );
          break;
        case "reject":
          enqueueSnackbar("This registration is currently in draft status.", {
            variant: "info",
            autoHideDuration: 3000,
          });
          break;
  ...
  const getConfirmationMessage = useCallback(() => {
    ...
    const messages = {
      approve:
        "Are you sure you want to approve this employee registration? This will move it from draft to approved status.",
      reject: (
        <>
...
export default PendingRegistrationDraft;
