...
const AttainmentDialog = ({ open, onClose, attainment }) => {
  ...
  } = useGetAttainmentAttachmentQuery(attainment?.id, {
    skip: !open || !attainment?.id || attainment?.id === "undefined",
  });
  ...
  // Set filename when attainment changes
  useEffect(() => {
    if (attainment) {
  ...
  // Function to get display filename (adapted for attainment)
  ...
  // If no attainment or no attachment, don't render
  if (!attainment || !attainment.attainment_attachment) {
    return null;
  }
...
export default AttainmentDialog;
