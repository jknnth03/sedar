...
const DataChangeCancelled = ({ searchQuery, dateFilters }) => {
  ...
  const handleRestoreSubmission = useCallback((submission) => {
    setSelectedSubmissionId(submission.id);
    setMenuAnchor({});
  ...
  const handleArchiveSubmission = useCallback((submission) => {
    setSelectedSubmissionId(submission.id);
    setMenuAnchor({});
    setSelectedSubmissionForAction(submission);
  ...
  const handleActionConfirm = async () => {
    ...
    try {
      if (confirmAction === "restore" && selectedSubmissionForAction) {
        enqueueSnackbar("Data change submission restored successfully!", {
          variant: "success",
          autoHideDuration: 2000,
        });
        refetch();
      } else if (confirmAction === "archive" && selectedSubmissionForAction) {
        enqueueSnackbar("Data change submission archived successfully!", {
          variant: "success",
          autoHideDuration: 2000,
        });
        refetch();
  ...
  const getConfirmationMessage = useCallback(() => {
    if (confirmAction === "restore") {
      return (
        <>
          Are you sure you want to <strong>Restore</strong> this Data Change
          Request?
        </>
      );
      ...
      return (
        <>
          Are you sure you want to <strong>Archive</strong> this Data Change
          Request?
        </>
      );
  ...
  const getConfirmationIcon = useCallback(() => {
    const iconConfig = {
      restore: { color: "#4caf50", icon: "↺" },
      archive: { color: "#795548", icon: "📁" },
      create: { color: "#4caf50", icon: "+" },
      update: { color: "#2196f3", icon: "✎" },
      resubmit: { color: "#ff9800", icon: "↻" },
    };

    const config = iconConfig[confirmAction] || iconConfig.restore;
    return config;
  }, [confirmAction]);
    ...
    <FormProvider {...methods}>
      <Box
        ...
        <DataChangeForapprovalTable
          ...
          handleRestoreSubmission={handleRestoreSubmission}
          handleArchiveSubmission={handleArchiveSubmission}
...
export default DataChangeCancelled;
