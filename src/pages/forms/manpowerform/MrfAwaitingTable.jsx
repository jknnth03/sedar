...
const MrfAwaitingTable = ({
  ...
  const awaitingSubmissions = useMemo(() => {
    return submissionsList.filter(
      (submission) =>
        submission.status?.toLowerCase() === "awaiting_resubmission" ||
        submission.status?.toLowerCase() === "awaiting resubmission"
    );
  }, [submissionsList]);
        ...
        <DialogContent sx={styles.historyDialogContent}>
          ...
          <Box sx={{ mb: 4 }}>
            <Box sx={{ mb: 4 }}>
              <Typography variant="h6" sx={styles.historyTitle}>
                ...
                ACTIVITY LOGS - AWAITING RESUBMISSION
                <PendingActionsIcon sx={styles.historyTimelineIcon} />
              </Typography>
          ...
          {selectedSubmissionHistory.activity_log.length === 0 && (
            <Box sx={styles.noHistoryContainer}>
              <PendingActionsIcon sx={styles.noHistoryIcon} />
        ...
        <Table stickyHeader sx={{ minWidth: 1000 }}>
          ...
          <TableBody>
            {isLoadingState ? (
              ...
              <TableRow>
                <TableCell colSpan={9} align="center" sx={styles.errorCell}>
              ...
              <TableRow>
                <TableCell
                  ...
                  <Box sx={styles.noDataBox}>
                    <PendingActionsIcon
                      sx={{ fontSize: 64, color: "#9c27b0", mb: 1 }}
                    />
                    <Typography variant="h6" color="text.secondary">
                      No submissions awaiting resubmission
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      {searchQuery
                        ? `No awaiting submissions found for "${searchQuery}"`
                        : "No submissions are currently awaiting resubmission"}
                    </Typography>
                  </Box>
                </TableCell>
              </TableRow>
...
export default MrfAwaitingTable;
