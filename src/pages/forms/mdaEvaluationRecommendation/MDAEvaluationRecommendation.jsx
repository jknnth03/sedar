...
import EvaluationForMDAProcessing from "./EvaluationForMDAProcessing";
import MDAEvaluationForApproval from "./MDAEvaluationForApproval";
import MDAEvaluationRejected from "./MDAEvaluationRejected";
import MDAEvaluationAwaitingResubmission from "./MDAEvaluationAwaitingResubmission";
import MDAEvaluationApproved from "./MDAEvaluationApproved";
import MDAEvaluationCancelled from "./MDAEvaluationCancelled";
import { useCancelFormSubmissionMutation } from "../../../features/api/forms/mdaEvaluationRecommendationApi";
...
const TabPanel = ({ children, value, index, ...other }) => {
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`mdaevaluation-tabpanel-${index}`}
      aria-labelledby={`mdaevaluation-tab-${index}`}
      style={{
        height: "100%",
        overflow: "hidden",
        minWidth: 0,
...
const CustomSearchBar = ({
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
      ...
      <TextField
        placeholder={isVerySmall ? "Search..." : "Search Evaluation..."}
        value={searchQuery}
...
const MDAEvaluationRecommendation = () => {
  ...
  const [cancelEvaluationSubmission] = useCancelFormSubmissionMutation();

  const debouncedSearchQuery = useDebounce(searchQuery, 500);

  const mdaEvaluationCounts = {
    forMDAProcessing: 0,
    forApproval: 0,
    rejected: 0,
  ...
  const handleCancel = useCallback(
    async (entryId, onSuccess) => {
      try {
        const result = await cancelEvaluationSubmission(entryId).unwrap();

        enqueueSnackbar("MDA Evaluation cancelled successfully!", {
          variant: "success",
        ...
        let errorMessage = "Failed to cancel MDA Evaluation. Please try again.";
    ...
    [cancelEvaluationSubmission, enqueueSnackbar]
  );
  ...
  const tabsData = [
    {
      ...
      badgeCount: mdaEvaluationCounts.forMDAProcessing,
    },
    {
      label: "FOR APPROVAL",
      component: (
        <MDAEvaluationForApproval
          searchQuery={debouncedSearchQuery}
          dateFilters={dateFilters}
          filterDataByDate={filterDataByDate}
          filterDataBySearch={filterDataBySearch}
      ...
      badgeCount: mdaEvaluationCounts.forApproval,
    },
    {
      label: "AWAITING RESUBMISSION",
      component: (
        <MDAEvaluationAwaitingResubmission
          searchQuery={debouncedSearchQuery}
          dateFilters={dateFilters}
          filterDataByDate={filterDataByDate}
          filterDataBySearch={filterDataBySearch}
      ...
      badgeCount: mdaEvaluationCounts.awaitingResubmission,
    },
    {
      label: "REJECTED",
      component: (
        <MDAEvaluationRejected
          searchQuery={debouncedSearchQuery}
          dateFilters={dateFilters}
          filterDataByDate={filterDataByDate}
          filterDataBySearch={filterDataBySearch}
      ...
      badgeCount: mdaEvaluationCounts.rejected,
    },
    {
      label: "APPROVED",
      component: (
        <MDAEvaluationApproved
          searchQuery={debouncedSearchQuery}
          dateFilters={dateFilters}
          filterDataByDate={filterDataByDate}
          filterDataBySearch={filterDataBySearch}
      ...
      badgeCount: mdaEvaluationCounts.approved,
    },
    {
      label: "CANCELLED",
      component: (
        <MDAEvaluationCancelled
          searchQuery={debouncedSearchQuery}
          dateFilters={dateFilters}
          filterDataByDate={filterDataByDate}
          filterDataBySearch={filterDataBySearch}
      ...
      badgeCount: mdaEvaluationCounts.cancelled,
    },
  ];

  const a11yProps = (index) => {
    return {
      id: `mdaevaluation-tab-${index}`,
      "aria-controls": `mdaevaluation-tabpanel-${index}`,
    };
  };
        ...
        <Box sx={styles.mainContainer}>
          <Box
            ...
            <Box
              ...
              <Typography
                ...
                {isVerySmall
                  ? "MDA (EVAL)"
                  : "MASTER DATA AUTHORITY (EVALUATION)"}
              </Typography>
            </Box>
          ...
          <Box sx={styles.tabsSection}>
            <StyledTabs
              ...
              aria-label="MDA Evaluation submissions tabs"
              variant="scrollable"
              scrollButtons="auto"
...
export default MDAEvaluationRecommendation;
