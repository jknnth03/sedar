...
const TabPanel = ({ children, value, index, ...other }) => {
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`catone-approval-tabpanel-${index}`}
      aria-labelledby={`catone-approval-tab-${index}`}
      style={{
        height: "100%",
        overflow: "hidden",
        minWidth: 0,
...
const CatOneApprovalTable = ({
  ...
  const {
    ...
    isFetching,
    error,
  } = useGetCatOneApprovalsQuery(queryParams, {
    ...
    <Box
      ...
      <TableContainer
        ...
        <Table
          ...
          <TableBody
            ...
            ) : catOneApprovalsList.length > 0 ? (
              catOneApprovalsList.map((approval) => {
                return (
                  <TableRow
                    key={approval.id}
      ...
      <Box
        ...
        <TablePagination
          ...
          component="div"
          count={catOneApprovalsData?.result?.total || 0}
...
const CatOneApproval = () => {
  ...
  const [approveCatOne, { isLoading: approveLoading }] =
    useApproveCatOneMutation();
  const [returnCatOne, { isLoading: returnLoading }] =
    useReturnCatOneMutation();

  const { data: selectedApprovalData, isLoading: selectedApprovalLoading } =
    useGetCatOneTaskByIdQuery(selectedApprovalId, {
  ...
  const handleReturn = useCallback(
    async ({ reason }) => {
      ...
      try {
        ...
        await returnCatOne(payload).unwrap();
        enqueueSnackbar("Category 1 returned successfully!", {
          variant: "success",
        });
        setDetailsDialog({ open: false, submission: null });
        setSelectedApprovalId(null);
      } catch (error) {
        enqueueSnackbar(error?.data?.message || "Failed to return Category 1", {
          variant: "error",
        });
      }
    },
  ...
  const a11yProps = (index) => {
    return {
      id: `catone-approval-tab-${index}`,
      "aria-controls": `catone-approval-tabpanel-${index}`,
    };
  };
      ...
      <Box sx={styles.mainContainer}>
        ...
        <Box sx={styles.tabsSection}>
          <StyledTabs
            ...
            aria-label="Category 1 Approval tabs"
            variant="scrollable"
            scrollButtons="auto"
