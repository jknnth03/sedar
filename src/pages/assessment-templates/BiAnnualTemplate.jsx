...
import BiAnnualTemplateModal from "../../components/modal/assessment-templates/BiAnnualTemplateModal";
import {
...
const BiAnnualTemplate = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetBiAnnualTemplatesQuery(queryParams);

  const templates = templatesResponse?.result?.data || [];
  const totalCount = templatesResponse?.result?.total || 0;

  const [updateTemplate] = useUpdateBiAnnualTemplateMutation();
...
export default BiAnnualTemplate;
