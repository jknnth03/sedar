...
const CustomSearchBar = ({
  ...
  return (
    <Box
      ...
      <TextField
        placeholder={isVerySmall ? "Search..." : "Search banks..."}
        value={searchQuery}
...
const Banks = () => {
  ...
  const {
    ...
    refetch,
    error,
  } = useGetShowBanksQuery(queryParams, {
    refetchOnMountOrArgChange: true,
  });

  const [deleteBank] = useDeleteBanksMutation();

  const banks = useMemo(() => backendData?.result?.data || [], [backendData]);
  ...
  const handleMenuOpen = useCallback((event, bank) => {
    event.stopPropagation();
    setMenuAnchor((prev) => ({ ...prev, [bank.id]: event.currentTarget }));
  }, []);

  const handleMenuClose = useCallback((bankId) => {
    setMenuAnchor((prev) => ({ ...prev, [bankId]: null }));
  }, []);
  ...
  const renderStatusChip = useCallback((bank) => {
    const isActive = !bank.deleted_at;
            ...
            <Table stickyHeader>
              ...
              <TableBody>
                {isLoadingState ? (
                  ...
                  banks.map((bank) => (
                    <TableRow key={bank.id} sx={styles.tableRowHover(theme)}>
                      <TableCell align="left">{bank.id}</TableCell>
                      <TableCell sx={styles.formNameCell}>
                        <Tooltip title={bank.code} placement="top">
                      ...
                      <TableCell sx={styles.formNameCell}>
                        <Tooltip title={bank.name} placement="top">
                          <span style={styles.cellContentStyles}>
                            {bank.name}
                          </span>
                        </Tooltip>
                      ...
                      <TableCell align="center">
                        ...
                        <Menu
                          anchorEl={menuAnchor[bank.id]}
                          open={Boolean(menuAnchor[bank.id])}
                          onClose={() => handleMenuClose(bank.id)}>
                          {!bank.deleted_at && (
                          ...
                          <MenuItem
                            onClick={(e) => handleArchiveRestoreClick(bank, e)}>
                            {bank.deleted_at ? (
                              <>
      ...
      <Dialog
        ...
        <DialogContent>
          <Typography variant="body1" gutterBottom textAlign="center">
            ...
            <strong>{selectedBank?.deleted_at ? "restore" : "archive"}</strong>{" "}
            this bank?
          </Typography>
          {selectedBank && (
            <Typography
