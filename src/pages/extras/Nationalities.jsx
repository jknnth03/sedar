...
import NationalitiesModal from "../../components/modal/extras/NationalitiesModal";
...
const CustomSearchBar = ({
  ...
  return (
    <Box
      ...
      <TextField
        placeholder={isVerySmall ? "Search..." : "Search nationalities..."}
        value={searchQuery}
...
const Nationalities = () => {
  ...
  const {
    ...
    refetch,
    error,
  } = useGetShowNationalitiesQuery(queryParams, {
    refetchOnMountOrArgChange: true,
  });

  const [deleteNationality] = useDeleteNationalitiesMutation();
  ...
  const handleMenuOpen = useCallback((event, nationality) => {
    event.stopPropagation();
    setMenuAnchor((prev) => ({
      ...prev,
      [nationality.id]: event.currentTarget,
    }));
  }, []);

  const handleMenuClose = useCallback((nationalityId) => {
    setMenuAnchor((prev) => ({ ...prev, [nationalityId]: null }));
  }, []);
  ...
  const handleArchiveRestoreClick = useCallback(
    (nationality, event) => {
      ...
      setConfirmOpen(true);
      handleMenuClose(nationality.id);
    },
    [handleMenuClose]
  );
  ...
  const handleArchiveRestoreConfirm = async () => {
    ...
    setIsLoading(true);
    try {
      await deleteNationality(selectedNationality.id).unwrap();
      enqueueSnackbar(
        selectedNationality.deleted_at
          ? "Nationality restored successfully!"
          : "Nationality archived successfully!",
  ...
  const renderStatusChip = useCallback((nationality) => {
    const isActive = !nationality.deleted_at;
                ...
                {isLoadingState ? (
                  ...
                  nationalities.map((nationality) => (
                    <TableRow
                      ...
                      <TableCell align="left">{nationality.id}</TableCell>
                      <TableCell sx={styles.formNameCell}>
                        <Tooltip title={nationality.code} placement="top">
                      ...
                      <TableCell sx={styles.formNameCell}>
                        <Tooltip title={nationality.name} placement="top">
                          <span style={styles.cellContentStyles}>
                            {nationality.name}
                          </span>
                        </Tooltip>
                      ...
                      {!isMobile && (
                        <TableCell align="center">
                          {renderStatusChip(nationality)}
                        </TableCell>
                      )}
                      <TableCell align="center">
                        <IconButton
                          onClick={(e) => handleMenuOpen(e, nationality)}
                          size="small">
                          <MoreVertIcon />
                        </IconButton>
                        <Menu
                          anchorEl={menuAnchor[nationality.id]}
                          open={Boolean(menuAnchor[nationality.id])}
                          onClose={() => handleMenuClose(nationality.id)}>
                          ...
                          <MenuItem
                            onClick={(e) =>
                              handleArchiveRestoreClick(nationality, e)
                            }>
                            {nationality.deleted_at ? (
                              <>
                  ...
                  <TableRow
                    ...
                    <TableCell
                      ...
                      <NoDataFound
                        ...
                        subMessage={
                          searchQuery
                            ? `No nationalities found for "${searchQuery}"`
      ...
      <Dialog
        ...
        <DialogContent>
          <Typography variant="body1" gutterBottom textAlign="center">
            ...
            <strong>
              {selectedNationality?.deleted_at ? "restore" : "archive"}
            </strong>{" "}
            this nationality?
          </Typography>
          {selectedNationality && (
