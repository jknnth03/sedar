...
const CustomSearchBar = ({
  ...
  return (
    <Box
      ...
      <TextField
        placeholder={isVerySmall ? "Search..." : "Search teams..."}
        value={searchQuery}
...
const Teams = () => {
  ...
  const {
    ...
    refetch,
    error,
  } = useGetShowTeamsQuery(queryParams, {
    refetchOnMountOrArgChange: true,
  });

  const [deleteTeam] = useDeleteTeamsMutation();

  const teams = useMemo(() => backendData?.result?.data || [], [backendData]);
  ...
  const handleMenuOpen = useCallback((event, team) => {
    event.stopPropagation();
    setMenuAnchor((prev) => ({ ...prev, [team.id]: event.currentTarget }));
  }, []);

  const handleMenuClose = useCallback((teamId) => {
  ...
  const renderStatusChip = useCallback((team) => {
            ...
            <Table stickyHeader>
              ...
              <TableBody>
                {isLoadingState ? (
                  ...
                  teams.map((team) => (
                    <TableRow key={team.id} sx={styles.tableRowHover(theme)}>
                      <TableCell align="left">{team.id}</TableCell>
                      <TableCell sx={styles.formNameCell}>
                        <Tooltip title={team.code} placement="top">
                      ...
                      <TableCell align="center">
                        ...
                        <Menu
                          ...
                          <MenuItem
                            onClick={(e) => handleArchiveRestoreClick(team, e)}>
                            {team.deleted_at ? (
                              <>
