...
import StatusModal from "../../components/modal/extras/StatusModal";
...
const CustomSearchBar = ({
  ...
  return (
    <Box
      ...
      <TextField
        placeholder={isVerySmall ? "Search..." : "Search status..."}
        value={searchQuery}
...
const Status = () => {
  ...
  const {
    ...
    refetch,
    error,
  } = useGetShowStatusQuery(queryParams, {
    refetchOnMountOrArgChange: true,
  });

  const [deleteStatus] = useDeleteExtraStatusMutation();
  ...
  const handleMenuOpen = useCallback((event, status) => {
    event.stopPropagation();
    setMenuAnchor((prev) => ({ ...prev, [status.id]: event.currentTarget }));
  }, []);
  ...
  const handleArchiveRestoreConfirm = async () => {
    ...
    setIsLoading(true);
    try {
      console.log("delete button hit", selectedStatus.id);
      const response = await deleteStatus(selectedStatus.id).unwrap();
      console.log(response, "dELETEEEEE");
      enqueueSnackbar(
  ...
  const renderStatusChip = useCallback((status) => {
    const isActive = !status.deleted_at;
            ...
            <Table stickyHeader>
              ...
              <TableBody>
                {isLoadingState ? (
                  ...
                  statuses.map((status) => (
                    <TableRow key={status.id} sx={styles.tableRowHover(theme)}>
                      <TableCell align="left">{status.id}</TableCell>
                      <TableCell sx={styles.formNameCell}>
                        <Tooltip title={status.code} placement="top">
                      ...
                      <TableCell align="center">
                        ...
                        <Menu
                          anchorEl={menuAnchor[status.id]}
                          open={Boolean(menuAnchor[status.id])}
                          onClose={() => handleMenuClose(status.id)}>
                          {!status.deleted_at && (
                            <MenuItem onClick={() => handleEditClick(status)}>
                          ...
                          <MenuItem
                            onClick={(e) =>
                              handleArchiveRestoreClick(status, e)
                            }>
                            {status.deleted_at ? (
                              <>
