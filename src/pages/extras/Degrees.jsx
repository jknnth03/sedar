...
const CustomSearchBar = ({
  ...
  return (
    <Box
      ...
      <TextField
        placeholder={isVerySmall ? "Search..." : "Search degrees..."}
        value={searchQuery}
...
const Degrees = () => {
  ...
  const {
    ...
    refetch,
    error,
  } = useGetShowDegreesQuery(queryParams, {
    refetchOnMountOrArgChange: true,
  });

  const [deleteDegree] = useDeleteDegreesMutation();

  const degrees = useMemo(() => backendData?.result?.data || [], [backendData]);
  ...
  const handleMenuOpen = useCallback((event, degree) => {
    event.stopPropagation();
    setMenuAnchor((prev) => ({ ...prev, [degree.id]: event.currentTarget }));
  }, []);

  const handleMenuClose = useCallback((degreeId) => {
    setMenuAnchor((prev) => ({ ...prev, [degreeId]: null }));
  }, []);
  ...
  const renderStatusChip = useCallback((degree) => {
    const isActive = !degree.deleted_at;
            ...
            <Table stickyHeader>
              ...
              <TableBody>
                {isLoadingState ? (
                  ...
                  degrees.map((degree) => (
                    <TableRow key={degree.id} sx={styles.tableRowHover(theme)}>
                      <TableCell align="left">{degree.id}</TableCell>
                      <TableCell sx={styles.formNameCell}>
                        <Tooltip title={degree.code} placement="top">
                      ...
                      <TableCell sx={styles.formNameCell}>
                        <Tooltip title={degree.name} placement="top">
                          <span style={styles.cellContentStyles}>
                            {degree.name}
                          </span>
                        </Tooltip>
                      ...
                      {!isMobile && (
                        <TableCell align="center">
                          {renderStatusChip(degree)}
                        </TableCell>
                      )}
                      <TableCell align="center">
                        <IconButton
                          onClick={(e) => handleMenuOpen(e, degree)}
                          size="small">
                          <MoreVertIcon />
                        </IconButton>
                        <Menu
                          anchorEl={menuAnchor[degree.id]}
                          open={Boolean(menuAnchor[degree.id])}
                          onClose={() => handleMenuClose(degree.id)}>
                          {!degree.deleted_at && (
                            <MenuItem onClick={() => handleEditClick(degree)}>
                          ...
                          <MenuItem
                            onClick={(e) =>
                              handleArchiveRestoreClick(degree, e)
                            }>
                            {degree.deleted_at ? (
                              <>
                  ...
                  <TableRow
                    ...
                    <TableCell
                      ...
                      <NoDataFound
                        ...
                        subMessage={
                          searchQuery
                            ? `No degrees found for "${searchQuery}"`
      ...
      <Dialog
        ...
        <DialogContent>
          <Typography variant="body1" gutterBottom textAlign="center">
            ...
            <strong>
              {selectedDegree?.deleted_at ? "restore" : "archive"}
            </strong>{" "}
            this degree?
          </Typography>
          {selectedDegree && (
